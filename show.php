...
<div class="watchlist">
    <div class="wl-title">
        можно посмотреть:<br><span>из категории сериалы</span>
    </div>
    ...
    <?php for ($i = $maxId; $i >= $maxId-7; $i--){?>
        <?php $result = $mysqli->query("SELECT * FROM `films` WHERE  id='$i'"); while($row = mysqli_fetch_assoc($result)) { $type= $row['type'];$id=$row['id']; }; if ($type=='сериал'){?>
            ...
            <div class="big-items-column">
                <div class="big-item-column">
                    ...
                    <div class="big-type-show">сериал</div>
